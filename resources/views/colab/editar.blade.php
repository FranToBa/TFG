        ...
        <form method="POST" action="{{ route($ruta) }}" enctype="multipart/form-data">
          ...
          <div class="form-group row">
              <label for="dni" class="col-md-4 col-form-label text-md-right">{{ __('Título') }}</label>
                  ...
                  <div class="col-md-6">
                      <input id="titulo" type="text" class="form-control @error('titulo') is-invalid @enderror" name="titulo" value="{{$objeto->titulo}}" required autofocus>
              ...
              <div class="form-group row ">
                ...
                <div class="col-xl-6 ">
                @if($objeto->foto and $titulo=="Editar noticia")
                  <img src="{{ route('noticia.foto', ['filename'=>$objeto->foto]) }}" class="foto_perfil"/>
                @elseif($objeto->foto and $titulo=="Editar evento")
                  <img src="{{ route('evento.foto', ['filename'=>$objeto->foto]) }}" class="foto_perfil"/>
                @endif
                </div>
              </div>
              ...
              @if($titulo=='Editar evento')
              <div class="form-group row">
                  <label for="dni" class="col-md-4 col-form-label text-md-right">{{ __('Fecha') }}</label>

                      <div class="col-md-6">
                          <input id="fecha" type="date" class="form-control @error('fecha') is-invalid @enderror" name="fecha" value="{{$objeto->fecha}}" required autofocus>
                  ...
                  <div class="form-group row">
                      <label for="dni" class="col-md-4 col-form-label text-md-right">{{ __('Aforo') }}</label>
                          ...
                          <div class="col-md-6">
                              <input id="aforo" type="number" min="0" class="form-control @error('aforo') is-invalid @enderror" name="aforo" value="{{$objeto->aforo}}" required autofocus>
          ...
          <!--  <textarea class="ckeditor" id="summary-ckeditor" name="summary-ckeditor" rows="10" cols="80"> -->
              <textarea class="form-control" id="summary-ckeditor" name="summary-ckeditor">{!! $objeto->descripcion !!}</textarea>
